  ...
  <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
    <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
      <div className="flex-1 flex flex-col justify-start mr-10">
        <a href="/">
          <img src={LogoNavbar} alt="pamm-fibox" className="w-[220px] object-contain" />
        </a>
          ...
          <div key={footerLink.key} className="flex flex-col ss:my-0 my-4 min-w-[150px]">
            ...
            <ul className="list-none mt-4">
              {footerLink.links.map((link, index) => (
                <li key={link.name} className={`font-poppins font-normal text-[16px] leading-[24px] text-dimWhite hover:text-secondary duration-300 cursor-pointer ${index !== footerLink.links.length - 1 ? "mb-4" : "mb-0"}`}>
                  {link.name}
                </li>
              ))}
            </ul>
